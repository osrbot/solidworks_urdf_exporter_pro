...
`f9b2ccc` 独立构建再次出现 3 个仿真对话框截图失败：中文 96 DPI 的 OpenUSD 选项区域高度为 1 像素，英文 96 / 144 DPI 的单位说明高度为 1 像素。上面的局部布局调整未能证明问题已解决，不能据单独截图测试或此前一次 997 项通过宣布修复完成。Core 129 项、官方 MuJoCo 9 项通过，但完整插件门禁未通过，不生成可交付安装包，不替换远端插件。远端 SolidWorks 进程仍在运行，尚未收到退出确认；未关闭进程，也未改动其装配体。下一步需定位完整测试序列中的布局状态，保留原断言，不绕过门禁。
