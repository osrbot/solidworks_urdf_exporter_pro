...
- Failed formats attempt to retain or restore their previous valid output. Retained output is marked as not updated this run. If recovery is required, inspect the directories as directed by the error details; do not assume the old output is intact.
...
- Cancellation or serious errors may interrupt remaining targets. Already committed outputs are retained; check each target's result.
