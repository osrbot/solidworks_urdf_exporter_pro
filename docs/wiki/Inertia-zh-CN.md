...
手动质心、惯性矩阵及其基准源会在预览或导出准备时转换，重复保存不会再次转换。
若原坐标系无法解析，会保留草稿并报错，需要先修复引用再继续。
...
输入的是整个 Link 的实测总质量，可对应多个零件；只影响插件配置、预览和导出，不写回 SW
零件或装配体，不更改材料、密度和覆盖设置。

直接输入质量并勾选校准，无需先重置。旧配置、手动矩阵和 SW 惯性覆盖值默认保留；启用时
确认使用 SW 惯性作为基准，保留输入的质量和质心，取消则保持原值。
“清除 Link 校准与手动修改”仅撤销插件内编辑并重新读取 SW 有效属性。
已打开的预览会刷新；页面、预览和导出共用最终数值，校准状态随配置保存。
