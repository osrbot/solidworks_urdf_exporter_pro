        ...
        private static IList<UrdfExportTutorialStep> BuildChinese()
        {
            return new List<UrdfExportTutorialStep>
            {
                ...
                new UrdfExportTutorialStep(
                    ...
                    "4. 需要完整交付时点击 导出 URDF 和网格；只有调试 XML 时才选择 导出 URDF（不含网格），该轻量兼容路径不生成 Bundle、Isaac 或新 profile。\r\n" +
                    "5. 选择一个可写的新目录，等待所有 Link 的网格和报告完成后再关闭 SolidWorks。\r\n" +
                    "6. 检查 Bundle/<包名>.osurdf，以及已选的 ROS1/<包名> 和 ROS2/<包名>。",
