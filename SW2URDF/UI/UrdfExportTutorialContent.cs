    ...
    internal static class UrdfExportTutorialContent
    {
        ...
        private static IList<UrdfExportTutorialStep> BuildChinese()
        {
            return new List<UrdfExportTutorialStep>
            {
                ...
                new UrdfExportTutorialStep(
                    ...
                    "4. # 数量代表 Link 深度。新 Link 自动生成 fixed Joint；camera_link 自动得到 camera_joint。\r\n" +
                    "5. 回到属性页，为新增或复制的 Link 分配 CAD 组件。复制结构不会复制组件绑定。",
                ...
                new UrdfExportTutorialStep(
                    ...
                    "3. 选择对应的 Origin_* 坐标系和 Axis，确认父 Link、子 Link 没有接反。\r\n" +
                    "4. revolute/prismatic 填写 lower、upper、effort、velocity；continuous 不填写上下位置限位。\r\n" +
                    "5. damping、friction、safety controller 只在有明确模型依据时填写。\r\n" +
                    "6. Mimic Joint 先选已存在的源 Joint，再设置 multiplier 和 offset，避免循环引用。",
                ...
                new UrdfExportTutorialStep(
                    ...
                    "5. 点击 显示惯性椭球，检查椭球中心、方向和尺度是否符合几何直觉。\r\n" +
                ...
                new UrdfExportTutorialStep(
                    "export",
                    "7. 同时导出 ROS1 与 ROS2 功能包",
                    "一次操作得到结构一致、网格齐全的 ROS1 和 ROS2 描述包。",
                    "1. ROS 包名使用小写字母、数字和下划线，例如 rover_description。\r\n" +
                    "2. 在 Link 属性页完成颜色、材质、网格格式、碰撞策略和精简比例设置。\r\n" +
                    "3. 需要完整包时点击 导出 URDF 和网格；仅调试 XML 时才选择 仅导出 URDF。\r\n" +
                    "4. 选择一个可写的新目录，等待所有 Link 的网格和报告完成后再关闭 SolidWorks。\r\n" +
                    "5. 输出应包含 ROS1/<包名> 和 ROS2/<包名>，两边都应有 urdf 与 meshes 内容。",
                    "ROS1/ROS2 都有 URDF；所有 mesh URI 都能找到对应文件；导出过程没有 timeout、fallback 或复制错误未处理。"),
                new UrdfExportTutorialStep(
                    "validate",
                    "8. 检查报告并在查看器验证",
                    ...
                    "解析无错误；视觉与碰撞齐全；COM/惯性椭球位置合理；关节方向和限位正确；ROS1/ROS2 结果一致。")
            };
        }
        ...
        private static IList<UrdfExportTutorialStep> BuildEnglish()
        {
            return new List<UrdfExportTutorialStep>
            {
                ...
                new UrdfExportTutorialStep("export", "7. Export ROS1 and ROS2 packages", "Produce matching ROS1 and ROS2 description packages with complete meshes.", "1. Use a lowercase package name with digits and underscores, such as rover_description.\r\n2. Finish materials, mesh format, collision strategy, and reduction settings.\r\n3. Choose Export URDF and Meshes for a complete package.\r\n4. Select a writable new directory and wait for all Link exports and reports.\r\n5. Confirm ROS1/<package> and ROS2/<package> both contain urdf and meshes.", "Both URDF files exist; every mesh URI resolves; no unhandled timeout, fallback, or copy error."),
